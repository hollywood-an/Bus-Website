...
import { roundCoord } from './useUserLocation';
...
export function usePlanner() {
  ...
  // Set when the origin is the user's actual position ("Your location" pick). Typing in the From
  // field clears it — text and coords never silently disagree.
  ...
  // `coords` (or the fromCoords state) switches the origin to real user coordinates.
  const plan = async (fromValue = fromLocation, toValue = toLocation, coords = fromCoords) => {
    if ((!coords && !fromValue.trim()) || !toValue.trim()) return;
    const seq = ++seqRef.current;
    setLoading(true);
    setError('');
    try {
      const query = coords
        ? `fromLat=${roundCoord(coords.lat)}&fromLng=${roundCoord(coords.lng)}&to=${encodeURIComponent(toValue)}`
        : `from=${encodeURIComponent(fromValue)}&to=${encodeURIComponent(toValue)}`;
      const res = await fetch(apiUrl(`/api/plan?${query}`), {
      ...
      else if (data.error === 'unresolved_from' && coords) {
        setError("You're outside the campus bus area (~3 mi around OSU), so trips can't start from your location. Type a campus starting point instead.");
  ...
  return { fromLocation, setFromLocation, toLocation, setToLocation, fromCoords, setFromCoords, trip, mode, setMode, loading, error, plan, prefill };
}
